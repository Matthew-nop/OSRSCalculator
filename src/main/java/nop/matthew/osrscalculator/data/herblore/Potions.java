...
public enum Potions implements Recipe {
	...
	GUTHIX_REST(
			...
			new ItemQuantity[0]), // prescription goggles don't work with Guthix rest
	HUNTER_POTION(
	...
	MAGIC_ESSENCE_POTION(
			...
			new ItemQuantity[0]), // TODO: Does this work with the amulet of chemistry or prescription goggles?
	MAGIC_POTION(
	...
	SERUM_207(
			...
			new ItemQuantity[0]), // Prescription goggles don't save ashes
	STAMINA_POTION1(
			new ItemQuantity(Item.STAMINA_POTION1, 1),
			77,
			25.5f,
	...
	private final ItemQuantity[] primaryIngredients;
	private final ItemQuantity[] secondaryIngredients;
	...
	Potions(
			...
			int level,
			float xp,
			ItemQuantity[] primaryIngredients,
			ItemQuantity[] secondaryIngredients,
			ItemQuantity amuletItemQuantity
	) {
		this.output = output;
		this.level = level;
		this.xp = xp;
	...
	Potions(
			...
			float xp,
			ItemQuantity[] primaryIngredients,
			ItemQuantity secondaryIngredient,
			ItemQuantity amuletItemQuantity
	) {
		this(output, level, xp, primaryIngredients, new ItemQuantity[]{secondaryIngredient}, amuletItemQuantity);
	}
	...
	Potions(
			...
			int level,
			float xp,
			ItemQuantity primaryIngredient,
			ItemQuantity[] secondaryIngredients,
			ItemQuantity amuletItemQuantity
	) {
		this(output, level, xp, new ItemQuantity[]{primaryIngredient}, secondaryIngredients, amuletItemQuantity);
	}
	...
	Potions(
		...
		this(output, level, xp, new ItemQuantity[]{primaryIngredient}, new ItemQuantity[]{secondaryIngredient}, amuletItemQuantity);
	}
	...
	Potions(
		...
		this(output, level, xp, primaryIngredients, secondaryIngredients, null);
	}
	...
	Potions(
		...
		this(output, level, xp, primaryIngredients, new ItemQuantity[]{secondaryIngredient}, null);
	}
	...
	Potions(
		...
		this(output, level, xp, new ItemQuantity[]{primaryIngredient}, secondaryIngredients, null);
	}
	...
	Potions(
		...
		this(output, level, xp, new ItemQuantity[]{primaryIngredient}, new ItemQuantity[]{secondaryIngredient}, null);
	}
	...
	public ItemQuantity[] getIngredients(Set<Flags> flags) {
		ItemQuantity[] ingredients = new ItemQuantity[primaryIngredients.length + secondaryIngredients.length];

		if (flags.contains(Flags.PRESCRIPTION_GOGGLES)) {
			for (int i = 0; i < ingredients.length; i++) {
				if (i < primaryIngredients.length) {
					ingredients[i] = primaryIngredients[i];
				}
				else {
					// Prescription goggles have a 10% chance to save secondary ingredients when creating potions
					ItemQuantity nextIngredient = secondaryIngredients[i - primaryIngredients.length];
					ingredients[i] = new ItemQuantity(nextIngredient.getItem(), nextIngredient.getQuantity() * 0.9f);
				}
			}
		}
		else {
			System.arraycopy(primaryIngredients, 0, ingredients, 0, primaryIngredients.length);
			System.arraycopy(secondaryIngredients, 0, ingredients, primaryIngredients.length, secondaryIngredients.length);
		}
	...
	public boolean flagAffects(Flags flag) {
		return (flag == Flags.AMULET_OF_CHEMISTRY && this.amuletItemQuantity != null)
				| (flag == Flags.PRESCRIPTION_GOGGLES && this.secondaryIngredients.length > 0);
	}
